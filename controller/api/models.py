...
# Extending BaseUserManager to customize create user and create superuser internally
# No fields only methods
class UserManager(BaseUserManager):
    def create_user(self, email, password=None, **extra_fields):
        if not email:
            raise ValueError('The Email must be set')
        email = self.normalize_email(email) # Uppercase to lower, Space removal etc.
        user = self.model(email=email, **extra_fields)
        user.set_password(password)  # Hashes the password
        user.save(using=self._db) # Default DB
        return user
