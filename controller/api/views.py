...
from django.contrib.auth import authenticate, login as auth_login # Name chnage due to conflict
...
def login(request):
    try:
        if request.method == 'POST':
            ...
            if user is not None:
                auth_login(request, user) # Set the cookies, logs in manages sessions etc.
                return JsonResponse({
